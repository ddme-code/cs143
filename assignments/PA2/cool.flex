...
[ \t\r\v\f]+	{}
  ...
  */
  ...
  /* the state change, one line note */
"--"			{ BEGIN LINE_COMMENT; }
  /* more line note */
"(\*"			{ BEGIN BLOCK_COMMENT; }
  /* the end of more line note without begin, error */
"\*)"			{
...
<BLOCK_COMMENT><<EOF>>	{ 
  ...
  /* in both kinds of note state, ignore the word*/
<LINE_COMMENT>.			{}
<BLOCK_COMMENT>.		{}
...
{LE}			  { return (LE); }
  ...
  */
  /* some symbol which should be reply original */
"{"			{ return '{'; }
"}"			{ return '}'; }
"("			{ return '('; }
")"			{ return ')'; }
...
"@"			{ return '@'; }
  ...
  */
  ...
  /* the key words that should be reply*/
